...
<div class="layui-form" style="width: 100%; margin-top: 20px;">
    <blockquote class="layui-elem-quote quoteBox">
        ...
        <button style="margin-left: 30px; margin-right: 20px;" id="searchBtn"
                class="layui-btn" type="button" lay-filter="userForm"><i class="layui-icon">&#xe615;</i>搜索
        </button>
        <shiro:hasPermission name="9001">
            <button id="addBtn" class="layui-btn layui-btn-norm al"><i class="layui-icon layui-icon-add-1"></i>添加
            </button>
        </shiro:hasPermission>
        <shiro:hasPermission name="80006">
            <button id="deleteBtn" class="layui-btn layui-btn-norm al"><i class="layui-icon layui-icon-delete"></i>删除
            </button>
            <button id="loseBtn" class="layui-btn layui-btn-norm al"><i class="layui-icon layui-icon-down"></i>失效
            </button>
        </shiro:hasPermission>
    ...
    layui.use(['table', 'form', 'laydate'], function () {
        ...
        var active = {
            reload: function () {
                ...
                var resultStatus = $("select[name='resultStatus']");
                //执行重载
                table.reload('saleOpportunityTable', {
                    page: {
                        curr: 1
        ...
        //批量删除销售机会（软删除）
        function loseOpportunities(ids) {
            var load = null;
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/opportunity/loseSaleOpportunities",
                data: {
                    "ids": ids
                },
                dataType: "json",
        ...
        $("#loseBtn").click(
            function () {
                ...
                var ids = '';
                if (data.length < 1) {
                    layer.msg('请选择要失效的销售机会！');
                } else {
                    layer.confirm(
                        '确定要失效' + data.length + '条销售机会吗？',
                        function (index) {
                            layer.close(index);
                            //循环读取数据，并把id通过-连接起来
                            for (i = 0; i < data.length; i++) {
